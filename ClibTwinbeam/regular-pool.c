...
int init₋regularpool(struct structa * 🅿)
{ unsigned bytes₋per₋tile=4096;
   if (structa₋init(4,bytes₋per₋tile,🅿)) { return -1; }
   return 0;
}

int optional₋uninit₋regularpool(struct structa * 🅿)
{
   if (deinit₋structa(🅿)) { return -1; }
   return 0;
}

int copy₋append₋onto₋regular(struct structa * 🅟, int32_t tetras, char32̄_t cs[])
